...
const ResultsScreen: React.FC<ResultsScreenProps> = ({ score, totalQuestions, onRetry, onHome, xpGained = 0, frGained = 0, wasRepeat = false, journeyLabel, journeyNext, onJourneyNext }) => {
  ...
  if (percentage >= 91) {
      resultType = 'gold';
      // Nota alta → Fred comemora junto com o aluno
      icon = <FredAvatar expression="feliz" className="w-40 h-48 drop-shadow-[0_0_25px_rgba(250,204,21,0.4)]" />;
      ...
      resultType = 'sad';
      // Nota baixa → em vez de uma carinha triste genérica, o Fred aparece
      // na pose motivadora: o objetivo é levantar o aluno, não puni-lo.
      icon = <FredAvatar expression="motivado" className="w-40 h-48 drop-shadow-[0_0_25px_rgba(247,147,30,0.3)]" />;
