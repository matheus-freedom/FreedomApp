...
const SelectionScreen: React.FC<SelectionScreenProps> = ({
  ...
  const canProceed = selectedTopic !== null && (selectedTopic !== CUSTOM_TOPIC_VALUE || customTopicText.trim().length > 2);
  const canStartMulti = isMultiSelectTheme ? selectedMultiTopics.size > 0 : canProceed;
    ...
    <div className="max-w-6xl mx-auto p-4 md:p-6 space-y-8 animate-fade-in relative">
      ...
      {showWelcomeModal && (
        <div className="fixed inset-0 z-[500] bg-black/90 backdrop-blur-xl flex items-center justify-center p-4">
          <div className="bg-[#2a2a2a] w-full max-w-lg rounded-[2.5rem] border-2 border-[#f7931e]/50 p-10 text-center space-y-8 animate-pop">
            ...
            <button onClick={() => { setShowWelcomeModal(false); localStorage.setItem(`welcome_seen_${user.userId}`, 'true'); }}
              className="w-full py-5 bg-[#f7931e] text-[#222222] rounded-2xl font-black uppercase tracking-widest hover:scale-105 transition-transform">
              Começar Agora
            </button>
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 md:gap-6">
        <div className="lg:col-span-2 bg-[#2a2a2a] rounded-[2rem] border border-white/5 p-6 shadow-xl flex flex-col md:flex-row gap-6 items-center relative overflow-hidden">
          ...
          <div className="flex-1 space-y-4 w-full">
            ...
            <div className="space-y-2">
              <div className="flex justify-between text-[8px] font-black uppercase tracking-widest text-gray-500">
                ...
                {nextTier && <span>Faltam {xpToNextLevel.toLocaleString()} XP</span>}
              </div>
              <div className="h-2 w-full bg-black/40 rounded-full overflow-hidden border border-white/5">
                <div className="h-full bg-[#f7931e] transition-all duration-1000" style={{ width: `${progressPercentage}%` }} />
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="bg-[#2a2a2a] rounded-[2rem] border border-white/5 p-6 shadow-xl flex flex-col justify-between">
          <div className="flex justify-between items-start mb-4">
            <div className="w-10 h-10 rounded-2xl bg-green-500/10 flex items-center justify-center text-green-400"><Wallet className="w-5 h-5" /></div>
        ...
        <div className="bg-[#2a2a2a] rounded-[2rem] border border-white/5 p-6 shadow-xl flex flex-col justify-between">
          <div className="flex justify-between items-start mb-4">
            <div className="w-10 h-10 rounded-2xl bg-blue-500/10 flex items-center justify-center text-blue-400"><Flame className="w-5 h-5" /></div>
            <div className="text-right">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <button onClick={onOpenStudyPlan} className={`p-5 rounded-[2rem] border-2 transition-all flex flex-col items-center justify-center gap-3 group relative overflow-hidden ${hasActivePlan ? 'bg-[#f7931e]/10 border-[#f7931e]/50' : 'bg-[#2a2a2a] border-white/5 hover:border-[#f7931e]/50'}`}>
          <div className={`p-4 rounded-2xl transition-colors ${hasActivePlan ? 'bg-[#f7931e] text-[#222222]' : 'bg-[#333333] text-[#f7931e] group-hover:bg-[#f7931e] group-hover:text-[#222222]'}`}><Calendar className="w-6 h-6" /></div>
        ...
        <button onClick={() => { if (isChallengeOnly) { setShowAccessAlert(true); return; } if (placementCooldownStatus.canTake) onStartPlacement(); }}
          ...
          {!placementCooldownStatus.canTake && (
            <div className="absolute inset-0 bg-[#222222]/80 flex flex-col items-center justify-center rounded-[2rem] opacity-0 group-hover:opacity-100 transition-opacity">
            ...
            {wizardStep === 3 && (
              <div className="animate-fade-in">
                ...
                <p className="text-[11px] text-gray-500 font-bold mb-4">
                  {isMultiSelectTheme ? 'Selecione até 2 tópicos para combinar.' : 'Selecione o assunto que quer praticar.'}
                </p>
                ...
                <div className="max-h-[220px] overflow-y-auto pr-1 mb-3 space-y-1.5 custom-scrollbar">
                  {availableTopics.map((topic) => {
                    ...
                    return (
                      <button key={topic} disabled={isDisabled}
                        onClick={() => { if (isMultiSelectTheme) { toggleMultiTopic(topic); } else { setSelectedTopic(topic); setCustomTopicText(''); } }}
                  ...
                  <button
                    onClick={() => { setSelectedTopic(CUSTOM_TOPIC_VALUE); setSelectedMultiTopics(new Set()); }}
                ...
                <div className="flex gap-3">
                  ...
                  <button onClick={handleStartClick} disabled={!canStartMulti || isLoading}
                    className="flex-1 py-4 bg-[#f7931e] text-[#222222] rounded-2xl font-black uppercase tracking-widest flex items-center justify-center gap-2 hover:scale-[1.02] transition-all shadow-xl shadow-[#f7931e]/20 disabled:opacity-40 disabled:cursor-not-allowed disabled:scale-100">
            ...
            {leaderboardUsers.map((lbUser, index) => {
              ...
              return (
                <div key={lbUser.user.userId} onClick={() => setViewProfileId(lbUser.user.userId)}
                  className={`flex items-center justify-between p-4 rounded-2xl border transition-all cursor-pointer hover:bg-white/5 ${isCurrentUser ? 'border-[#f7931e]/50' : rankStyle}`}>
